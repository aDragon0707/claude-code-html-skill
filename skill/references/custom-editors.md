...
Official examples export the thing the user needs next, not every possible thing:

- Triage board: `Copy as markdown` exports planning state grouped by bucket.
...
- `Copy selected tasks`.
- `Copy Now bucket`.
- `Copy full board as Markdown`.
- `Copy Obsidian sync note`.

Do not make a single "next Codex prompt" that includes every card unless the user explicitly wants a full batch.
...
If the board is for execution, add per-card or selected-item exports so the user can run one task at a time.
